...
function dealGame(drawMode) {
  const mode = drawMode || 1
...
function useLayout() {
  ...
  function calcLayout() {
    ...
    return { W, H, fs, ss, faceDownH, faceUpOverlap, gap, vw }
  }
  useEffect(() => {
...
function Card({ card, selected, onClick, W, H, fs, ss, style={} }) {
  ...
  return (
    <div onClick={onClick} style={{
      ...
      boxShadow: selected ? '0 0 0 2px rgba(201,168,76,0.5),0 6px 16px rgba(0,0,0,0.35)' : '0 2px 5px rgba(0,0,0,0.2)',
      cursor:onClick?'pointer':'default', userSelect:'none', position:'relative',
      transform:selected?'translateY(-8px)':'none', transition:'transform 0.12s,box-shadow 0.12s',
      ...style
    }}>
...
function Slot({ label, onClick, W, H }) {
  ...
  return (
    <div onClick={onClick} style={{
      width:W, height:H, borderRadius:6, border:`2px dashed ${bc}`,
      background:'rgba(255,255,255,0.03)', display:'flex', alignItems:'center', justifyContent:'center',
      color:tc, fontSize:H>70?'1.4rem':'1rem', cursor:onClick?'pointer':'default', flexShrink:0,
    }}>{label}</div>
  )
}
  ...
  return (
    <div style={{position:'fixed',inset:0,pointerEvents:'none',zIndex:300,overflow:'hidden'}}>
      {Array.from({length:60}).map((_,i) => (
        <div key={i} style={{
          position:'absolute', left:`${Math.random()*100}%`, top:`-${Math.random()*20+5}%`,
...
function findHint(game) {
  ...
  if (waste.length > 0) {
    ...
    for (let fi=0;fi<4;fi++) if (canPlaceOnFoundation(card,foundations[fi],fi)) return `Move ${card.value}${card.suit} from waste → foundation`
  }
  for (let ci=0;ci<7;ci++) {
    ...
    for (let fi=0;fi<4;fi++) if (canPlaceOnFoundation(card,foundations[fi],fi)) return `Move ${card.value}${card.suit} from column ${ci+1} → foundation`
  }
  if (waste.length > 0) {
    const card = waste[waste.length-1]
    for (let ci=0;ci<7;ci++) if (canPlaceOnTableau(card,tableau[ci])) return `Move ${card.value}${card.suit} from waste → column ${ci+1}`
  }
  ...
  if (stock.length > 0) return 'Draw from stock'
  if (waste.length > 0) return 'Reset stock and draw'
...
function HintButton({ game }) {
  ...
  function showHint() {
    setHint(findHint(game)); setShowing(true)
...
function NewGameDialog({ onStart, onCancel, showCancel }) {
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',zIndex:400,display:'flex',alignItems:'center',justifyContent:'center',padding:'1rem'}}>
      <div style={{background:'linear-gradient(135deg,#1a3d28,#0f2a1a)',border:'2px solid #c9a84c',borderRadius:20,padding:'2rem 1.75rem',maxWidth:420,width:'100%',textAlign:'center'}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:12,marginBottom:'1.75rem'}}>
          {[
            { mode:1, label:'Draw 1', tag:'Beginner Friendly', tagColor:'#5DCAA5', tagBg:'rgba(93,202,165,0.15)',
              stats:[{val:'~80%',sub:'Theoretically winnable',color:'#5DCAA5'},{val:'~40%',sub:'Average player win rate',color:'#c9a84c'}],
              desc:'One card flipped at a time. More moves available, easier to plan ahead.' },
            { mode:3, label:'Draw 3', tag:'Classic / Hard', tagColor:'#e74c3c', tagBg:'rgba(192,57,43,0.15)',
              stats:[{val:'~20%',sub:'Theoretically winnable',color:'#c0392b'},{val:'~8%',sub:'Average player win rate',color:'#c9a84c'}],
              desc:'Three cards flipped at a time. Only every third card is accessible. Casino style.' },
          ].map(({ mode, label, tag, tagColor, tagBg, stats, desc }) => (
            <div key={mode} onClick={() => setDrawMode(mode)} style={{
              background:drawMode===mode?'rgba(201,168,76,0.15)':'rgba(255,255,255,0.04)',
              border:`2px solid ${drawMode===mode?'#c9a84c':'rgba(255,255,255,0.1)'}`,
              borderRadius:12,padding:'1rem 1.25rem',cursor:'pointer',textAlign:'left',transition:'all 0.15s',
            }}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
                <div style={{fontWeight:700,color:'var(--cream)',fontSize:'1rem'}}>{label}</div>
                <span style={{fontSize:'0.65rem',padding:'2px 8px',borderRadius:20,background:tagBg,color:tagColor,fontWeight:600}}>{tag}</span>
              </div>
              <div style={{display:'flex',gap:16,marginBottom:6}}>
                {stats.map(s => (
                  <div key={s.sub} style={{textAlign:'center'}}>
                    <div style={{fontSize:'1.1rem',fontWeight:800,color:s.color}}>{s.val}</div>
                    <div style={{fontSize:'0.6rem',color:'rgba(245,240,232,0.4)'}}>{s.sub}</div>
                  </div>
                ))}
              </div>
              <p style={{fontSize:'0.78rem',color:'rgba(245,240,232,0.45)',lineHeight:1.5,margin:0}}>{desc}</p>
            </div>
          ))}
        </div>
        <div style={{display:'flex',gap:'0.75rem',justifyContent:'center'}}>
          <button onClick={() => onStart(drawMode)} className="btn-gold" style={{fontSize:'0.95rem',padding:'0.7rem 2rem'}}>
            Deal Cards ♣
          </button>
          {showCancel && (
            <button onClick={onCancel} style={{padding:'0.7rem 1.5rem',borderRadius:8,background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.15)',color:'rgba(245,240,232,0.6)',cursor:'pointer',fontSize:'0.88rem'}}>
              Cancel
            </button>
          )}
        </div>
...
export default function Solitaire() {
  ...
  function tryAutoFoundation(card, sourceType, sourceCol) {
    for (let fi=0;fi<4;fi++) {
      if (canPlaceOnFoundation(card, game.foundations[fi], fi)) {
        ...
        setScore(s => s+15); setMoves(m => m+1); setSelected(null)
        return true
      }
    }
    return false
  ...
  function drawStock() {
    ...
    setGame(g => {
      ...
      if (ng.stock.length === 0) {
        // Reset: Draw1 reverses, Draw3 shuffles
        const cards = ng.waste.map(c => ({...c, faceUp:false}))
        if (ng.drawMode === 3) {
  ...
  function handleWaste() {
    ...
    if (!top) return
    const now = Date.now(); const key = 'waste'
    if (now-(lastClickTime[key]||0)<350) {
      setLastClickTime(l=>({...l,[key]:0}))
      tryAutoFoundation(top,'waste',null)
      return
    }
  ...
  function handleTableau(colIdx, cardIdx) {
    if (!game) return
    const col = game.tableau[colIdx]; const card = col[cardIdx]
    if (!card.faceUp) {
      if (cardIdx!==col.length-1) return
      saveHist()
      setGame(g=>{const ng=JSON.parse(JSON.stringify(g));ng.tableau[colIdx][cardIdx].faceUp=true;return ng})
      setScore(s=>s+5); setMoves(m=>m+1); setSelected(null)
      return
    }
    const now=Date.now(); const key=`${colIdx}-${cardIdx}`; const isTop=cardIdx===col.length-1
    if (isTop&&now-(lastClickTime[key]||0)<350) {
      setLastClickTime(l=>({...l,[key]:0}))
    ...
    setLastClickTime(l=>({...l,[key]:now}))
    if (selected) {
      if (selected.source.type==='tableau'&&selected.source.col===colIdx&&cardIdx>=selected.source.cardIdx) { setSelected(null); return }
  ...
  function colHeight(col) {
    if (!col.length) return H
    return col.filter(c=>!c.faceUp).length*faceDownH + col.filter(c=>c.faceUp).length*faceUpOverlap + H
  }

  function renderWaste() {
    if (!game) return null
    const waste = game.waste
    if (!waste.length) return <Slot label="" W={W} H={H} />
    if ((game.drawMode||1)===1) {
    ...
    return (
      <div style={{position:'relative',width:W+(visible.length-1)*fanOffset,height:H,flexShrink:0}} onClick={handleWaste}>
        {visible.map((card,i) => (
          <div key={card.id} style={{position:'absolute',left:i*fanOffset,zIndex:i}}>
            <Card card={card} selected={i===visible.length-1&&wasteSel} W={W} H={H} fs={fs} ss={ss} />
          </div>
        ))}
      </div>
    )
  ...
  return (
    <div style={{paddingTop:56,height:'100vh',display:'flex',flexDirection:'column',background:'#0d4a2a',overflow:'hidden',userSelect:'none'}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:'0.6rem'}}>
          ...
          {!isMobile && (
            <span style={{fontFamily:"'Playfair Display',serif",color:'#c9a84c',fontWeight:700}}>
              ♣ Solitaire
              <span style={{fontSize:'0.65rem',color:'rgba(245,240,232,0.35)',marginLeft:6,fontFamily:'sans-serif',fontWeight:400}}>Draw {game.drawMode||1}</span>
            </span>
          )}
        </div>
      ...
      {/* New game dialog mid-game */}
      {showNewGameDialog && game && (
        <NewGameDialog onStart={startNewGame} showCancel onCancel={()=>setShowNewGameDialog(false)} />
      )}
      ...
      <div style={{flex:1,display:'flex',overflow:'hidden'}}>
        <div style={{flex:1,overflow:'auto',padding:`${isMobile?'0.5rem':'0.75rem'} ${isMobile?'0.4rem':'0.75rem'}`}}>
          ...
          {/* Top row */}
          <div style={{display:'flex',gap,marginBottom:isMobile?'0.5rem':'0.75rem',alignItems:'flex-start',width:'100%'}}>
            <div onClick={drawStock} style={{cursor:'pointer',flex:1,maxWidth:W,position:'relative'}}>
              ...
              <div style={{textAlign:'center',fontSize:'0.55rem',color:'rgba(245,240,232,0.3)',marginTop:2}}>
                {game.stock.length>0 ? game.stock.length : game.waste.length>0 ? 'Reset' : 'Empty'}
              </div>
            </div>
            <div onClick={(game.drawMode||1)===1?handleWaste:undefined} style={{cursor:'pointer',flex:1,maxWidth:(game.drawMode||1)===3?W*1.5:W}}>
              {renderWaste()}
            </div>
              ...
              return (
                <div key={colIdx} style={{flex:1,maxWidth:W,minWidth:W,position:'relative',height:colHeight(col)+10}}>
                  {col.map((card,cardIdx)=>{
                    const pfd=col.slice(0,cardIdx).filter(c=>!c.faceUp).length
                    const pfu=col.slice(0,cardIdx).filter(c=>c.faceUp).length
                    const top=pfd*faceDownH+pfu*faceUpOverlap
                    return (
                      <div key={card.id} style={{position:'absolute',top,left:0,zIndex:cardIdx+1}} onClick={()=>handleTableau(colIdx,cardIdx)}>
        ...
        {!isMobile && (
          <div style={{width:200,background:'rgba(0,0,0,0.3)',borderLeft:'1px solid rgba(201,168,76,0.1)',padding:'1rem 0.85rem',display:'flex',flexDirection:'column',gap:12,flexShrink:0,overflowY:'auto'}}>
            ...
            <div style={{background:'rgba(0,0,0,0.25)',borderRadius:8,padding:'10px'}}>
              ...
              {[['Mode',`Draw ${game.drawMode||1}`],['Score',score],['Moves',moves],['Time',fmt(time)],['Stock left',game.stock.length]].map(([l,v])=>(
                <div key={l} style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
                  <span style={{fontSize:'0.72rem',color:'rgba(245,240,232,0.5)'}}>{l}</span>
                  <span style={{fontSize:'0.78rem',color:'#c9a84c',fontWeight:700}}>{v}</span>
                </div>
              ))}
            </div>
            ...
            <div style={{background:'rgba(0,0,0,0.15)',borderRadius:8,padding:'10px'}}>
              ...
              <p style={{fontSize:'0.7rem',color:'rgba(245,240,232,0.4)',lineHeight:1.5}}>
                Build 4 piles A→K by suit. Descending order, alternating red/black. Kings only in empty columns. Double-click to auto-send to foundation.
              </p>
            </div>
          </div>
        )}
